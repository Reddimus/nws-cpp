...
int main() {
	...
	const std::chrono::seconds interval{60};

	std::cout << "Monitoring station " << station << " (" << poll_count << " observations, "
			  << interval.count() << "s interval)\n\n";

	for (int i = 0; i < poll_count; ++i) {
		nws::Result<nws::ObservationResponse> obs = client.get_latest_observation(station);
		if (!obs) {
			...
			if (p.temperature.has_value()) {
				double f = nws::celsius_to_fahrenheit(*p.temperature.value);
				std::cout << " | " << static_cast<int>(f) << "F";
			}
			if (p.wind_speed.has_value()) {
				nws::QuantitativeValue mph = p.wind_speed.converted_to(nws::Unit::Mi_h);
