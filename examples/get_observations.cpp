...
int main() {
	...
	nws::Result<nws::ObservationResponse> obs = client.get_current_weather(37.7749, -122.4194);
	if (!obs) {
		std::cerr << "Error: " << obs.error().message << "\n";
		return 1;
	}

	const nws::ObservationProperties& p = obs->properties;
	...
	if (p.temperature.has_value()) {
		nws::QuantitativeValue temp_f = p.temperature.converted_to(nws::Unit::DegF);
	...
	if (p.wind_speed.has_value()) {
		nws::QuantitativeValue wind_mph = p.wind_speed.converted_to(nws::Unit::Mi_h);
	...
	if (p.barometric_pressure.has_value()) {
		nws::QuantitativeValue mb = p.barometric_pressure.converted_to(nws::Unit::Millibar);
	...
	if (p.visibility.has_value()) {
		nws::QuantitativeValue miles = p.visibility.converted_to(nws::Unit::Mile);
